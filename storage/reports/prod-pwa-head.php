...
$staffShareMeta = $staffShareMeta ?? (basename((string) ($_SERVER['SCRIPT_NAME'] ?? '')) === 'staff-app.php');
if ($staffShareMeta && $pdo instanceof PDO) {
    require_once __DIR__ . '/share-meta.php';
    $staffShareSite = getSiteName($pdo);
    renderShareMeta([
        'title'       => 'Sign in — ' . $staffShareSite,
        'description' => $staffShareSite . ' — shifts, check-in, messages',
        'site_name'   => $staffShareSite,
        'url'         => getAbsolutePublicUrl('/staff-app.php', $pdo),
    ], $pdo);
}
?>
<?php if ($enablePwa): ?>
