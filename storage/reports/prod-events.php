...
$filteredEvents   = [];
$view             = 'all';
$viewCounts       = ['all' => 0, 'upcoming' => 0, 'past' => 0, 'active' => 0, 'inactive' => 0];
$flash            = getAdminFlash();
$sheetStatus      = ['total' => 0, 'missing' => 0];
$hasSa            = false;
$hasOauth         = false;
...
// Drive picker is optional — never block the events page on Google API calls.
$driveSheets   = [];
...
foreach ($allEvents as $event) {
    ...
    error_log('[EventStaff] events.php: ' . $e->getMessage() . ' in ' . $e->getFile() . ':' . $e->getLine());
    $eventsPageError = 'Events could not load fully. Try refreshing. If this continues, open System health or contact support.';
}
...
<?php endif; ?>
<?php if ($eventsPageError !== ''): ?>
    <div class="alert alert--error alert--visible"><?= h($eventsPageError) ?></div>
<?php endif; ?>
