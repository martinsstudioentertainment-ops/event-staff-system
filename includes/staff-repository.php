...
function getDashboardStats(PDO $pdo): array
{
    ...
    $safeCount = static function (string $sql) use ($pdo): int {
        try {
            ...
            error_log('[EventStaff] getDashboardStats: ' . $e->getMessage());
...
function updateStaffStatus(PDO $pdo, int $id, string $status, bool $allowIncompleteApproval = false): bool
{
    ...
    if ($status === 'approved' && !$allowIncompleteApproval) {
        ...
        if ($row !== null) {
            ...
            canonicalIdentityEnforceOnRegistration($pdo, $id, 'status_approve');
            ...
            $eventId = (int) ($row['event_id'] ?? 0);
            if ($staffId > 0 && $eventId > 0) {
                $conflict = canonicalIdentityFindActiveRegistrationForStaffOnEvent($pdo, $staffId, $eventId, $id);
                if ($conflict !== null && (string) ($conflict['status'] ?? '') === 'approved') {
                    return false;
                }
            }
        }
    ...
    if ($status === 'approved') {
        require_once __DIR__ . '/platform/canonical-identity.php';
        canonicalIdentityEnforceOnRegistration($pdo, $id, 'status_approve_final');
    }
...
{
    require_once __DIR__ . '/platform/canonical-identity.php';
    canonicalIdentityGatewayPush('change_staff_email');
    try {
...
function findOrCreateStaff(PDO $pdo, array $data): int
{
    ...
    // Try to find existing staff by canonical identity (email, PPSN, mobile, PSA).
    require_once __DIR__ . '/platform/canonical-identity.php';
