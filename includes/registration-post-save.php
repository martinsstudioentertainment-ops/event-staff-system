...
/** Slow registration follow-up (email, sheets queue, auto-approval) — run after HTTP response. */
...
function runRegistrationPostSaveJobs(PDO $pdo, array $data, array $ids, array $newEventIds, string $email): array
{
    ...
    try {
        ...
        staffPreferencesSaveFromRegistrationIfPresent($pdo, $data);
    } catch (Throwable $e) {
        error_log('[EventStaff] Registration preferences save: ' . $e->getMessage());
    }
