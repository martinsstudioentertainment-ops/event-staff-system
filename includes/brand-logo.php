...
function getCompanyLogoUrl(?PDO $pdo, string $assetBase = ''): string
{
    ...
    $absolute = getCompanyLogoAbsoluteUrl($pdo);
    if ($absolute !== '') {
        return $absolute;
    }
...
/**
 * Public absolute URL for emails and external clients (marketing site first).
 */
function getCompanyLogoAbsoluteUrl(PDO $pdo): string
{
    $relative = getCompanyLogoRelativePath($pdo);
    if ($relative === '' || getCompanyLogoFilesystemPath($pdo) === '') {
        return '';
    ...
    $bases = array_values(array_unique(array_filter([
        getMarketingSiteUrl($pdo),
        getRegistrationSiteUrl($pdo),
        getAdminSiteUrl($pdo),
    ])));
