...
function buildEmailMasterLayout(PDO $pdo, string $title, string $innerHtml, array $options = []): string
{
    ...
    if ($hasHeroBanner) {
        $bannerBlock = '<tr><td style="padding:0;line-height:0;font-size:0;background-color:' . emailEsc($primary) . ';">'
            . '<img src="' . emailEsc($brand['banner_url']) . '" width="600" height="400" alt="' . emailEsc($brand['company_name'] . ' — ' . $brand['event_staff_label']) . '" style="display:block;width:100%;max-width:600px;height:auto;border:0;outline:none;">'
            . '</td></tr>';
    } elseif ($showBanner) {
...
function buildEmailOtpContent(PDO $pdo, string $code, string $username, int $ttlMinutes = 10, string $intro = ''): string
{
    $brand = getEmailBranding($pdo);
    if ($intro === '') {
        $intro = 'Use this verification code to complete your sign-in:';
    }

    return '<p style="margin:0 0 16px;">' . emailEsc($intro) . '</p>'
