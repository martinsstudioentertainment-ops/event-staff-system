...
function renderStaffPortalShiftBanner(PDO $pdo, ?array $portalStaff): void
{
    ...
    $isActive  = staffShiftMonitoringIsActive($shift);
    ?>
    <div class="staff-v2__alert staff-v2__alert--<?= $isActive ? 'success' : 'info' ?>" role="status" id="staff-shift-banner">
        <?php if ($isActive): ?>
            <strong>On shift — <?= h($eventName) ?></strong><br>
            Stay signed in on this phone. GPS tracks you inside the <?= (int) $radius ?> m venue zone; leaving signs you out automatically.
            Add this app to your home screen and reopen it during the shift if you switch apps.
        <?php else: ?>
            <strong>Checked in — <?= h($eventName) ?></strong><br>
            Attendance activates when the event starts. Keep the staff app open on this phone so GPS can verify you are still at the venue.
        <?php endif; ?>
    </div>
    <?php
}
...
function renderStaffPortalTodayPayrollNote(PDO $pdo, ?array $portalStaff): void
{
    ...
    ?>
    <div class="staff-v2__alert staff-v2__alert--info" role="status">
        <strong><?= h($eventName) ?> — payroll</strong><br>
        Payable hours: <strong><?= h(formatHoursDecimal($paid)) ?></strong>
        <?php if ($note !== ''): ?>
            <br><?= h($note) ?>
        <?php endif; ?>
    </div>
