...
/**
 * Render one compact WhatsApp card for a single event group.
 */
function renderStaffEventWhatsappCard(string $eventName, string $eventDate, string $groupUrl): void
{
    $title = $eventName;
    ...
    ?>
<section class="wa-join-card wa-join-card--compact wa-join-card--event" aria-label="<?= htmlspecialchars('WhatsApp group for ' . $eventName, ENT_QUOTES, 'UTF-8') ?>">
    <div class="wa-join-card__icon" aria-hidden="true">
        <svg viewBox="0 0 24 24" width="28" height="28" fill="currentColor"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347z"/><path d="M12 2C6.477 2 2 6.477 2 12c0 1.89.525 3.66 1.438 5.168L2 22l4.832-1.438A9.955 9.955 0 0 0 12 22c5.523 0 10-4.477 10-10S17.523 2 12 2zm0 18a7.96 7.96 0 0 1-4.075-1.125l-.293-.175-2.87.85.85-2.87-.175-.293A7.96 7.96 0 0 1 4 12c0-4.411 3.589-8 8-8s8 3.589 8 8-3.589 8-8 8z"/></svg>
    </div>
    <div class="wa-join-card__body">
        <h2 class="wa-join-card__title"><?= htmlspecialchars($title, ENT_QUOTES, 'UTF-8') ?></h2>
        <p class="wa-join-card__text">Join this event&apos;s WhatsApp group for shift updates and last-minute calls.</p>
        <div class="wa-join-card__actions">
 ...
 * @return int Number of cards rendered
 */
function renderStaffEventWhatsappGroupsFromShiftRows(array $shiftRows): int
{
    ...
    foreach ($shiftRows as $row) {
        ...
        $groupUrl = normalizeWhatsappGroupUrl((string) ($row['whatsapp_group_url'] ?? ''));
        if ($groupUrl === '') {
            continue;
        }
        ...
        $eventDate = formatEventDateLabel((string) ($row['event_date'] ?? ''));
        renderStaffEventWhatsappCard($eventName, $eventDate, $groupUrl);
        $rendered++;
    }
 ...
 * @return int Number of cards rendered
 */
function renderStaffEventWhatsappGroups(PDO $pdo, string $email): int
{
    require_once dirname(__DIR__) . '/event-whatsapp.php';
...
/**
 * Staff app: per-event WhatsApp cards only (no company-wide fallback).
 *
 * @param array{shift_rows?: list<array<string, mixed>>, allow_company_fallback?: bool} $options
 */
function renderStaffAppWhatsappSection(PDO $pdo, string $email, array $options = []): void
{
    ...
    $shiftRows = $options['shift_rows'] ?? null;
    if (is_array($shiftRows) && $shiftRows !== []) {
        if (renderStaffEventWhatsappGroupsFromShiftRows($shiftRows) > 0) {
            return;
        }
    }

    if (renderStaffEventWhatsappGroups($pdo, $email) > 0) {
        return;
    }

    if (!empty($options['allow_company_fallback'])) {
        renderWhatsappGroupCard($pdo, 'compact');
    }
}

/**
 * Render wrapped section only when at least one event WhatsApp card is shown.
 *
 * @param array{shift_rows?: list<array<string, mixed>>, allow_company_fallback?: bool} $options
 */
function renderStaffAppWhatsappSectionBlock(PDO $pdo, string $email, array $options = [], string $ariaLabel = 'WhatsApp groups'): void
{
    ob_start();
    renderStaffAppWhatsappSection($pdo, $email, $options);
    $html = ob_get_clean();
    if (trim($html) === '') {
        return;
    }
    ?>
<section class="es-v3__section" aria-label="<?= htmlspecialchars($ariaLabel, ENT_QUOTES, 'UTF-8') ?>">
    <?= $html ?>
</section>
    <?php
}
