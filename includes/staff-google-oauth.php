...
/** Registration uses the same Gmail gate as staff sign-in when Google is enabled. */
function isRegistrationGoogleRequired(?PDO $pdo = null): bool
{
    return isStaffGoogleSigninEnabled($pdo);
}
...
function staffGoogleFetchProfileFromAccessToken(string $accessToken): array
{
    ...
    $data = json_decode($responseBody, true);
    if (!is_array($data)) {
        return ['ok' => false, 'message' => 'Could not read your Gmail address from Google.', 'email' => ''];
    }

    $verified = $data['verified_email'] ?? $data['email_verified'] ?? false;
    if (!$verified) {
        return ['ok' => false, 'message' => 'Your Google email is not verified. Verify it in Gmail settings, then try again.', 'email' => ''];
    }
...
function authenticateStaffPortalByGoogleEmail(PDO $pdo, string $email): array
{
    ...
    if ($staff === null) {
        return [
            'ok'      => false,
            'message' => 'This Gmail is not registered yet. Register for an event first using the same email address.',
        ];
    }
    ...
    if ($regCount < 1 && (int) ($staff['profile_completed'] ?? 0) !== 1) {
        return [
            'ok'      => false,
            'message' => 'This Gmail is not on our staff list yet. Register for a shift first, then sign in with Google.',
        ];
    }
...
function getRegistrationVerifiedGoogleEmail(): ?string
{
    ...
    $at    = (int) ($_SESSION['registration_google_verified_at'] ?? 0);
...
{
    unset($_SESSION['registration_google_email'], $_SESSION['registration_google_verified_at']);
}

/**
 * Verify Gmail for new registration (no existing staff record required).
 *
...
function completeRegistrationGoogleVerify(PDO $pdo, string $code): array
{
    ...
    if ($email === '' || !filter_var($email, FILTER_VALIDATE_EMAIL)) {
        return ['ok' => false, 'message' => 'Could not read your Gmail address from Google.'];
    }
