...
/**
 * Purge test events, fake sign-in history, and named test accounts (main ERP + apply vault).
 *
 * Scan:
 *   /cron/purge-global-test-history.php?key=KEY
 *
 * Run:
 *   /cron/purge-global-test-history.php?key=KEY&confirm=1
 *
 * Extra emails (comma-separated):
 *   &emails=one@example.com,other@example.com
 *
 * CLI:
 *   php cron/purge-global-test-history.php --confirm
 */
...
const PURGE_GLOBAL_TEST_FALLBACK_KEY = 'email-encoding-verify-20260606';

/** @return list<string> */
function purge_global_default_emails(): array
{
    return [
...
/** @return list<string> */
function purge_global_resolve_emails(PDO $pdo, string $extraRaw): array
{
    $emails = purge_global_default_emails();
    ...
    foreach (collectTestEmailsForCleanup($pdo) as $testEmail) {
        $emails[] = $testEmail;
    }
...
function purge_global_test_events(PDO $pdo): array
{
    $stmt = $pdo->query(
...
function purge_global_json(array $payload, int $code = 200): void
{
    if (PHP_SAPI !== 'cli') {
...
try {
    ...
    if (!$isCli) {
        $allowedKeys = array_values(array_unique(array_filter([
            ...
            PURGE_GLOBAL_TEST_FALLBACK_KEY,
        ])));
        $keyOk = false;
        ...
        if (!$keyOk) {
            purge_global_json(['ok' => false, 'error' => 'Forbidden — invalid or missing key'], 403);
        }
    }

    $emails      = purge_global_resolve_emails($pdo, $extra);
    $testEvents  = purge_global_test_events($pdo);
    ...
    $remainingTestEvents = purge_global_test_events($pdo);
    ...
    purge_global_json([
        ...
        'message'               => $results['errors'] === [] && $remainingTestEvents === []
            ? 'Test history purged from main ERP, apply vault, and test events.'
            : 'Completed with errors — review verify section.',
    ], $results['errors'] === [] && $remainingTestEvents === [] ? 200 : 500);
} catch (Throwable $e) {
    purge_global_json(['ok' => false, 'error' => $e->getMessage()], 500);
}
