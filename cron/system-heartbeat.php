...
try {
    ...
    $noShowStats = ['marked' => 0, 'skipped' => 0, 'blacklisted' => 0];
    try {
        require_once dirname(__DIR__) . '/includes/attendance-repository.php';
        $noShowStats = closeAwaitingSigninsAsNoShows($pdo);
    } catch (Throwable $e) {
        error_log('[EventStaff] system-heartbeat no-show close: ' . $e->getMessage());
    }

    systemHeartbeatArmNext($pdo, (int) ($sheetsStats['pending'] ?? 0));

    $line = sprintf(
        "[%s] system-heartbeat — sheets processed=%d success=%d pending=%d; attendance activated=%d; no_shows marked=%d\n",
        date('Y-m-d H:i:s'),
        (int) ($sheetsStats['processed'] ?? 0),
        (int) ($sheetsStats['success'] ?? 0),
        (int) ($sheetsStats['pending'] ?? 0),
        $attendanceActivated,
        (int) ($noShowStats['marked'] ?? 0)
    );
