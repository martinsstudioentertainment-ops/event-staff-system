...
(function () {
    ...
    function psaRequired() {
        return body.dataset.psaRequired !== '0';
    }

    function effectiveTotal() {
        return psaRequired() ? TOTAL : TOTAL - 1;
    }
    ...
    function updateChrome() {
        ...
        var displayTotal = fastTrackActive ? FAST_TRACK_TOTAL : effectiveTotal();
        var displayCurrent = fastTrackActive ? fastTrackDisplayStep(current) : displayStepNumber(current);
        ...
        document.querySelectorAll('[data-step-dot]').forEach(function (dot) {
            ...
            dot.classList.toggle('reg-wizard__dot--psa-hidden', !psaRequired() && n === 7);
        });
    ...
    function goNext() {
        ...
        if (!fastTrackActive && (current === TOTAL - 1 || (!psaRequired() && current === 6))) {
            var validation = window.RegistrationWizardValidation;
        ...
        form.addEventListener('submit', function (e) {
            ...
            if (!valid) {
                ...
                if (validation && typeof validation.hasValidEventSelection === 'function' && !validation.hasValidEventSelection()) {
                    ...
                    if (consentErr && consentErr.classList.contains('form-error--visible')) {
    ...
    function applyVerifiedGoogleEmail() {
        ...
        var hiddenEl = document.getElementById('registration_verified_email')
