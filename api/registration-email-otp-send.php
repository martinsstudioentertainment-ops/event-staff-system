...
try {
    ...
    if ($blocked !== null) {
        ...
        error_log('[EventStaff] registration-email-otp-send: blocked code=' . (string) ($blocked['code'] ?? 'GOOGLE_REQUIRED'));
    ...
    if (!getStaffAuthPolicy($pdo)['registration_email_otp_enabled']) {
        ...
        error_log('[EventStaff] registration-email-otp-send: OTP disabled by policy');
    ...
    if (empty($result['ok'])) {
        ...
        if (($result['code'] ?? '') === 'OTP_SEND_FAILED') {

            error_log('[EventStaff] registration-email-otp-send: OTP_SEND_FAILED');
