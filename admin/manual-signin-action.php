...
$result = recordAdminManualCheckinBulk($pdo, $eventId, $toProcess, $note, (int) $adminUser['id'], $bibMap);
...
foreach ($toProcess as $regId => $hours) {
    ...
    logAdminAudit(
        ...
        'registration',
        (int) $regId,
        'Manual sign-in with ' . $hours . 'h'
            . (($bibMap[$regId] ?? $bibMap[(string) $regId] ?? '') !== '' ? ' · bib #' . trim((string) ($bibMap[$regId] ?? $bibMap[(string) $regId] ?? '')) : '')
            . ($note !== '' ? ' — ' . $note : '')
    );
