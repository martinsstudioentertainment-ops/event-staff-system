...
register_shutdown_function(static function (): void {
    ...
    echo '<!DOCTYPE html><html lang="en"><head><meta charset="utf-8"><title>Event Hub error</title></head><body style="font-family:system-ui,sans-serif;padding:2rem;max-width:40rem">';
    echo '<h1>Event Hub could not load</h1><p>Send this to support:</p><pre style="background:#f1f5f9;padding:1rem;border-radius:8px;overflow:auto">';
...
if (!isFeatureEnabled($pdo, 'event_hub')) {
    setAdminFlash('warning', 'Event Hub flag is off — page still loads. Turn on “Event Hub” in Feature flags if you use this screen daily.');
}
...
try {
    ...
    error_log('[EventStaff] event-hub.php snapshot: ' . $e->getMessage());
    setAdminFlash('error', 'Event Hub data could not load: ' . $e->getMessage());
}
