# Phase 12B — Production QA audit fixes deploy
#
#   powershell -ExecutionPolicy Bypass -File .\scripts\deploy-phase12b-qa-audit.ps1
...
$backupRoot = Join-Path $ProjectRoot "storage\backups\phase12b-pre-deploy-$stamp"
...
$approvedFiles = @(
    ...
    @{ Local = 'includes/admin-login-otp.php'; Remote = 'includes/admin-login-otp.php' }
)
...
Write-Host '  Phase 12B — QA Audit Fixes Deploy' -ForegroundColor Green
Write-Host '========================================' -ForegroundColor Green
Write-Host ''

Write-Host '[1/7] Deploy safety gate ...' -ForegroundColor Green
...
Write-Host ''
Write-Host '[2/7] Phase 12B regression tests ...' -ForegroundColor Green
php (Join-Path $ProjectRoot 'scripts\phase12b-qa-audit-test.php')
if ($LASTEXITCODE -ne 0) {
    throw 'Phase 12B tests failed.'
}

php (Join-Path $ProjectRoot 'scripts\phase12a-status-v3-test.php')
if ($LASTEXITCODE -ne 0) {
    throw 'Phase 12A regression failed.'
}

php (Join-Path $ProjectRoot 'scripts\phase12-registration-500-fix-test.php')
if ($LASTEXITCODE -ne 0) {
    throw 'Phase 12 registration regression failed.'
}
...
Write-Host ''
Write-Host '[3/7] Pre-deploy backup ...' -ForegroundColor Green
...
$manifest = [ordered]@{
    phase           = 'PHASE12B-QA-AUDIT'
    timestamp       = (Get-Date).ToString('o')
    backup_dir      = $backupRoot
    backup_status   = $backupStatus
    production_hash = $hashBefore
    local_hash      = $hashLocal
    root_cause      = 'MobileOtpService sent full HTML documents bypassing branded email layout (no banner, footer, dark mode)'
}
$manifest | ConvertTo-Json -Depth 5 | Set-Content (Join-Path $backupRoot 'manifest.json') -Encoding UTF8

Write-Host ''
Write-Host '[4/7] Upload approved files ...' -ForegroundColor Green
foreach ($entry in $approvedFiles) {
    $local = Join-Path $ProjectRoot $entry.Local
...
Write-Host ''
Write-Host '[5/7] Verify uploaded files ...' -ForegroundColor Green
...
foreach ($entry in $approvedFiles) {
    ...
    $verifyDirParent = Split-Path $verifyPath -Parent
    if (-not (Test-Path $verifyDirParent)) {
        New-Item -ItemType Directory -Force -Path $verifyDirParent | Out-Null
    }
...
Write-Host ''
Write-Host '[6/7] Production HTTP verification ...' -ForegroundColor Green
powershell -ExecutionPolicy Bypass -File (Join-Path $ProjectRoot 'scripts\phase12b-post-deploy-verify.ps1')
if ($LASTEXITCODE -ne 0) {
    throw 'Phase 12B post-deploy verification failed.'
}

Write-Host ''
Write-Host '[7/7] Full production route probe ...' -ForegroundColor Green
powershell -ExecutionPolicy Bypass -File (Join-Path $ProjectRoot 'scripts\phase12-full-audit-probe.ps1')
if ($LASTEXITCODE -ne 0) {
    Write-Host 'Route probe reported issues — review docs/phase12-audit-probe-*.json' -ForegroundColor Yellow
}
...
Write-Host ''
Write-Host 'Phase 12B deploy complete.' -ForegroundColor Green
