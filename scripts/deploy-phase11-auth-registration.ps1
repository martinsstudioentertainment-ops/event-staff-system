# Phase 11 — Auth & registration modernization deploy
#
#   powershell -ExecutionPolicy Bypass -File .\scripts\deploy-phase11-auth-registration.ps1
...
$backupRoot = Join-Path $ProjectRoot "storage\backups\phase11-pre-deploy-$stamp"
...
$approvedFiles = @(
    ...
    @{ Local = 'assets\css\registration-v3.css'; Remote = 'assets/css/registration-v3.css' },
...
Write-Host '  Phase 11 — Auth & Registration Deploy' -ForegroundColor Green
Write-Host '========================================' -ForegroundColor Green
Write-Host ''
...
Write-Host ''
Write-Host '[2/6] Phase 11 static regression tests ...' -ForegroundColor Green
php (Join-Path $ProjectRoot 'scripts\phase11-auth-registration-test.php')
if ($LASTEXITCODE -ne 0) {
    throw 'Phase 11 auth/registration tests failed.'
}
...
if ($LASTEXITCODE -ne 0) {
    throw 'Phase 7 design system regression failed.'
}
...
$manifest = [ordered]@{
    phase           = 'PHASE11-AUTH-REGISTRATION'
...
powershell -ExecutionPolicy Bypass -File (Join-Path $ProjectRoot 'scripts\phase11-post-deploy-verify.ps1')
if ($LASTEXITCODE -ne 0) {
    throw 'Phase 11 post-deploy verification failed.'
}
