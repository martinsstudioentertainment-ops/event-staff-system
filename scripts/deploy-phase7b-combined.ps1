# Phase 7B — Combined deploy (Phase 7 Design System + Phase 5C Login Parity shell/JS).
#
#   powershell -ExecutionPolicy Bypass -File .\scripts\deploy-phase7b-combined.ps1
...
$backupRoot = Join-Path $ProjectRoot "storage\backups\phase7b-pre-deploy-$stamp"
$reportPath = Join-Path $ProjectRoot "docs\PHASE7B-DEPLOYMENT-REPORT.md"
...
$approvedFiles = @(
    ...
    @{ Local = 'offline.php'; Remote = 'offline.php' }
)
...
Write-Host '  Phase 7B — Combined Deploy (Option B)' -ForegroundColor Green
Write-Host '========================================' -ForegroundColor Green
Write-Host ''
...
Write-Host ''
Write-Host '[2/6] Pre-deploy login parity static checks ...' -ForegroundColor Green
php (Join-Path $ProjectRoot 'scripts\phase5c-login-parity-test.php')
if ($LASTEXITCODE -ne 0) {
    throw 'Phase 5C login parity tests failed.'
}
php (Join-Path $ProjectRoot 'scripts\phase7-design-system-test.php')
if ($LASTEXITCODE -ne 0) {
    throw 'Phase 7 design system tests failed.'
}

Write-Host ''
Write-Host '[3/6] Pre-deploy backup (production FTP snapshot) ...' -ForegroundColor Green
...
$manifest = [ordered]@{
    phase           = 'PHASE7B-COMBINED-OPTION-B'
...
Write-Host ''
Write-Host '[4/6] Local file hashes (to deploy) ...' -ForegroundColor Green
foreach ($rel in $hashLocal.Keys | Sort-Object) {
    Write-Host "  $rel" -ForegroundColor Gray
    Write-Host "    SHA256: $($hashLocal[$rel])" -ForegroundColor DarkGray
}

Write-Host ''
Write-Host '[5/6] Upload approved files to production ...' -ForegroundColor Green
foreach ($entry in $approvedFiles) {
    $local = Join-Path $ProjectRoot $entry.Local
...
Write-Host ''
Write-Host '[6/6] Verify uploaded files on production ...' -ForegroundColor Green
...
Write-Host ''
Write-Host 'Phase 7B deploy complete.' -ForegroundColor Green
