# Screenshot post-fix SMTP MIME inbox previews (Gmail / Outlook / Android).
$ErrorActionPreference = 'Stop'
$Root = Split-Path $PSScriptRoot -Parent
$HtmlDir = Join-Path $Root 'docs\screenshots\email-smtp-fix\html'
$OutDir = Join-Path $Root 'docs\screenshots\email-smtp-fix'
...
$templates = @('registration_confirmation', 'registration_approved', 'rejection_email', 'admin_alert')
$clients = @(
    @{ Id = 'gmail'; W = 620; H = 900 },
    @{ Id = 'outlook'; W = 720; H = 900 },
    @{ Id = 'android'; W = 390; H = 820 }
)
