# Phase 12A — Application Status page v3 completion deploy
#
#   powershell -ExecutionPolicy Bypass -File .\scripts\deploy-phase12a-status-v3.ps1
...
$backupRoot = Join-Path $ProjectRoot "storage\backups\phase12a-pre-deploy-$stamp"
...
Write-Host '  Phase 12A — Status Page V3 Deploy' -ForegroundColor Green
Write-Host '========================================' -ForegroundColor Green
Write-Host ''
...
Write-Host ''
Write-Host '[2/7] Phase 12A regression tests ...' -ForegroundColor Green
php (Join-Path $ProjectRoot 'scripts\phase12a-status-v3-test.php')
if ($LASTEXITCODE -ne 0) {
    throw 'Phase 12A tests failed.'
}
...
$manifest = [ordered]@{
    phase           = 'PHASE12A-STATUS-V3'
    timestamp       = (Get-Date).ToString('o')
    backup_dir      = $backupRoot
    backup_status   = $backupStatus
    production_hash = $hashBefore
    local_hash      = $hashLocal
    root_cause      = 'Phase 10 moved status.php to v3 shell but staff-status-dashboard.css not loaded; metrics/cards lacked v3 grid/card layout'
}
...
powershell -ExecutionPolicy Bypass -File (Join-Path $ProjectRoot 'scripts\phase12a-post-deploy-verify.ps1')
if ($LASTEXITCODE -ne 0) {
    throw 'Phase 12A post-deploy verification failed.'
}

Write-Host ''
Write-Host '[7/7] Phase 12 regression ...' -ForegroundColor Green
...
Write-Host ''
Write-Host 'Phase 12A deploy complete.' -ForegroundColor Green
